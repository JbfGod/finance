          ...
          {row.rowNum > 0 && (
            <Badge dot={row.formula}>
              <a onClick={() => formulaModal.open({
                rowNum: idx + 1, changeFormula: formulaHandler(row.id),
                        ...
                        toolBarRender={() => {
                          return [
                            <Button type="primary" key="copy" onClick={() => {
                              navigator.clipboard.writeText(window.btoa(JSON.stringify(dataSource)))
                            ...
                            <Button type="primary" key="paste" onClick={() => {
                              navigator.clipboard.readText()
                                .then(clipText => {
                                  try {
                                    const array = JSON.parse(window.atob(clipText))
