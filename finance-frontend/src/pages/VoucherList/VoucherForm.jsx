...
export default ({modal, onSuccess, subjects, setSubjects, subjectById, defaultVoucherDate, ...props}) => {
    ...
    /*const swapAmount = (index) => {
        const items = formRef.getFieldValue(["items"])
        formRef.setFieldsValue({
            items: items.map((item, idx) => {
                let {debitAmount, creditAmount} = item
                if (idx === index || index == null) {
                    return {...item, creditAmount: debitAmount, debitAmount: creditAmount}
                }
                return item
            })
        })
    ...
    const columns = [
        ...
        ...(isForeignCurrency ? [
            ...
            {
                title: "贷方金额",
                editable: (_, row) => !isFromExpenseBill || row.lendingDirection !== "BORROW",
                valueType: "digit",
                dataIndex: "creditAmount",
                fieldProps: {style: {width: "100%"}},
                render: (_, row) => {
        ...
        ]),
        {
            ...
            render: (_, row, index) => {
                if (isFromExpenseBill && row.lendingDirection === "BORROW") {
                    return null
                }
                return (
